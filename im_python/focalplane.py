...
class DataProcessor:
    def __init__(self, path, run, filename):
        self.path = path
    ...
    def load_data(self):
        ...
        """
        Path_data = f'{self.path}/{self.num_run}/{self.filename}'
...
class FocalPlanePlotter:
    ...
    def border(self):
        ...
        self.df_file['new_raft'] = self.df_file['raft'] + \
            '_' + self.df_file['sensor']
    ...
    def bord(self, ax):
        ...
        """
        # vertical line
        ax.axvline(x=0 - 0.5, ymin=6/30, ymax=6 *
                   4/30, color='black', linewidth=2)
        ax.axvline(x=8*3 - 0.5, ymin=0, ymax=6/30, color='black', linewidth=2)
        ax.axvline(x=8*3 - 0.5, ymin=6*4/30, ymax=6 *
                   5/30, color='black', linewidth=2)
        ax.axvline(x=8*12 - 0.5, ymin=0, ymax=6/30, color='black', linewidth=2)
        ax.axvline(x=8*12 - 0.5, ymin=6*4/30, ymax=6 *
                   5/30, color='black', linewidth=2)
        ax.axvline(x=8*15 - 0.5, ymin=6/30, ymax=6 *
                   4/30, color='black', linewidth=2)
        ax.axvline(x=8*14 - 0.5, ymin=24/30, ymax=26 /
                   30, color='black', linewidth=2)
        ax.axvline(x=8*13 - 0.5, ymin=26/30, ymax=28 /
                   30, color='black', linewidth=2)
        ax.axvline(x=8*13 - 0.5, ymin=2/30, ymax=4 /
                   30, color='black', linewidth=2)
        ax.axvline(x=8*14 - 0.5, ymin=4/30, ymax=6 /
                   30, color='black', linewidth=2)
        ax.axvline(x=8 - 0.5, ymin=24/30, ymax=26 /
                   30, color='black', linewidth=2)
        ax.axvline(x=8*2 - 0.5, ymin=26/30, ymax=28 /
                   30, color='black', linewidth=2)
        ax.axvline(x=8*2 - 0.5, ymin=2/30, ymax=4 /
                   30, color='black', linewidth=2)
        ax.axvline(x=8 - 0.5, ymin=4/30, ymax=6/30, color='black', linewidth=2)
        # horizontal line
        ax.axhline(y=0 - 0.5, xmin=8*3/120, xmax=8 *
                   3*4/120, color='black', linewidth=2)
        ax.axhline(y=6 - 0.5, xmin=0, xmax=8*3/120, color='black', linewidth=2)
        ax.axhline(y=6 - 0.5, xmin=8*3*4/120, xmax=8 *
                   3*5/120, color='black', linewidth=2)
        ax.axhline(y=24 - 0.5, xmin=0, xmax=8*3 /
                   120, color='black', linewidth=2)
        ax.axhline(y=24 - 0.5, xmin=8*3*4/120, xmax=8 *
                   3*5/120, color='black', linewidth=2)
        ax.axhline(y=30 - 0.5, xmin=8*3/120, xmax=8 *
                   3*4/120, color='black', linewidth=2)
        ax.axhline(y=2 - 0.5, xmin=16/120, xmax=24 /
                   120, color='black', linewidth=2)
        ax.axhline(y=4 - 0.5, xmin=8/120, xmax=16 /
                   120, color='black', linewidth=2)
        ax.axhline(y=26 - 0.5, xmin=8/120, xmax=16 /
                   120, color='black', linewidth=2)
        ax.axhline(y=28 - 0.5, xmin=16/120, xmax=24 /
                   120, color='black', linewidth=2)
        ax.axhline(y=2 - 0.5, xmin=24*4/120, xmax=8 *
                   13/120, color='black', linewidth=2)
        ax.axhline(y=4 - 0.5, xmin=8*13/120, xmax=8 *
                   14/120, color='black', linewidth=2)
        ax.axhline(y=26 - 0.5, xmin=8*13/120, xmax=8 *
                   14/120, color='black', linewidth=2)
        ax.axhline(y=28 - 0.5, xmin=8*12/120, xmax=8 *
                   13/120, color='black', linewidth=2)
    ...
    def focalplane_faulty(self, ax):
        ...
        self.df = self.df.merge(
            data, how='inner', on=self.merge_columns).copy()
...
class GainStudy:
    ...
    def faultyCCDs(self, fabr):
        ...
        self.dff = self.df[self.df['raft'].isin(
            self.fabr)].reset_index(drop=True)
